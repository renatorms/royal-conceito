...
- **Textos institucionais:** as páginas "Quem Somos", "Política de Privacidade", "Contato", "Termos de Uso" e "Política de Trocas e Devoluções" (rodapé do site) têm só texto genérico de exemplo, não o texto real da loja — "Contato" em especial tem telefone/WhatsApp/e-mail/endereços das lojas com valores claramente fictícios, aguardando os dados reais do cliente. **A Política de Privacidade e os Termos de Uso em especial não podem ir ao ar como estão** — ambos têm implicação jurídica real e precisam ser revisados por um advogado ou pelo próprio cliente antes do lançamento.
...
- Desde 15/08, o padrão do site é o tema claro (fundo branco); o tema escuro usado até então continua definido no CSS (bloco `.dark` em `index.css`), só não é mais ativado automaticamente — ainda não existe um toggle claro/escuro.
...
- **Frontend:** `/privacidade` e `/termos-de-uso` têm conteúdo genérico e **não podem ir ao ar como estão** — precisam revisão jurídica antes de produção; `/sobre`, `/contato` e `/politica-de-trocas` também são texto placeholder (`/contato` em especial tem dados de contato/endereço fictícios, aguardando informação real do cliente); Header ainda não tem tratamento responsivo completo no cluster de ações à direita. Ver as entradas de `Sobre.jsx`/`Privacidade.jsx`/`Contato.jsx`/`TermosDeUso.jsx`/`PoliticaTrocas.jsx`/`Header.jsx` em `docs/frontend.md`.
