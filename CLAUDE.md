...
- **`src/api/`** — one function-per-resource module per REST resource, using the shared `api` axios instance; `produtos.js` is the first one: `listarProdutos({ categoria, marca, search, ordering, page })`, `buscarProduto(id)`, `listarCategorias()`, `listarMarcas()` — the last two follow the API's `next` link to the end since `/categorias/` and `/marcas/` are paginated too (10/page) and the catalog currently has more than 10 of each
  - `enderecos.js` — `listarEnderecos()` (same follow-`next`-to-the-end pagination pattern, in case a user ever has 10+ saved addresses), `criarEndereco(dados)`, `atualizarEndereco(id, dados)` (`PATCH /enderecos/{id}/`, not `PUT` — `EnderecoViewSet` doesn't override `perform_update()` the way it overrides `perform_create()`, so a full `PUT` would need `usuario` resent in the payload since the serializer marks it required; a partial `PATCH` only validates the fields actually sent, so the update path can skip `usuario` entirely and leave it untouched), `deletarEndereco(id)` (`DELETE /enderecos/{id}/`)
...
- `Endereco.usuario` has no `blank=True`/`null=True` on the model, so `EnderecoSerializer` (fields = `"__all__"`) marks it required even though `EnderecoViewSet.perform_create()` immediately overwrites it with `request.user`. The frontend sends `usuario: user.id` in the `POST /enderecos/` payload purely to satisfy validation — a backend fix (`blank=True, null=True`, or dropping `usuario` from the serializer fields) would let the client omit it, matching how `Pedido.usuario` already works. The same required-field quirk applies to updates too, but `EnderecoViewSet` doesn't override `perform_update()` at all (only `perform_create()`), so there's no equivalent server-side re-assignment of `usuario` on edit — a full `PUT` would need the client to resend a correct `usuario`, which is one more way to get it wrong (e.g. accidentally reassigning the address to a different user id, since nothing server-side stops it). `atualizarEndereco()` sidesteps this entirely by using `PATCH`: DRF's partial update only validates fields actually present in the payload, so the frontend never sends `usuario` on edit and the field simply stays unchanged.
