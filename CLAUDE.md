...
- **`PedidoConfirmado`** — a small inline placeholder component in `App.jsx` (same pattern as the existing `MeusPedidos` placeholder), shown after a successful checkout; will grow into a real order confirmation view during the "Área do Cliente" phase
...
Not yet done: no protected views beyond the `Meus Pedidos` placeholder consume real API data yet; `Header` is intentionally minimal and will be redesigned; real shipping cost (SuperFrete) and payment gateway integration are separate pending decisions — checkout currently uses a fixed placeholder shipping value and does not process payment.
